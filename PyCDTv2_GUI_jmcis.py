...
# Version2.0: Adding two more algorithms: SMA (spectral mixture anaylsis), and CVA (change vector analysis) and fixing other bugs from version 1. 
...
import numpy as np
import PyCDTv2_Algorithms_jmcis as cdalg
import PyCDTv2_Functions_jmcis as cdfn
import time
...
def CD_algorithm():
    ...
    img_multi_0 = cdfn.read_envi_img(file_name_1)
    img_multi_1 = cdfn.read_envi_img(file_name_2)
    ...
    ###############--------Algorithm indices--------######
    # #indices [0-ALL algorithms, 1-PDTL, 2-Complexity, 3-Mean-shift, 4-Outlier-Shift, 5-Graph-based, 6-Chronochrome,
    # 7-Covariance Equalization, 8-PCA, 9-Image Diff & Image Ratioing, 10-CVA, 11-SMA.
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[1]==1:
        ...
        img_out_1 = cdalg.pdtl_cd(img_multi_0, img_multi_1, Tile_size=Tile_size)
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[2]==1:
        print('Complexity....')
        xy_startend = [0, 0, height, width]
        img_out_2 = cdalg.complexity_cd(img_multi_0, img_multi_1, Tile_size=Tile_size)
        # ^^^^^^Image_output_display^^^^^^#
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[3]==1:
        print('Mean....')
        xy_startend = [0, 0, height, width]
        img_out_3 = cdalg.meanshift_cd(img_multi_0, img_multi_1, Tile_size=Tile_size)
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[4]==1:
        print('Outlier....')
        xy_startend = [0, 0, height, width]
        img_out_4 = cdalg.outliershift_cd(img_multi_0, img_multi_1, Tile_size=Tile_size)
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[5]==1:
        ...
        xy_startend = [0,0,height,width]
        K_NN = 30
        img_out = cdalg.graph_theoretic(img_multi_0, img_multi_1,xy_startend,Tile_size=Tile_size,K_NN = K_NN)
        # ^^^^^^Image_output_display^^^^^^#
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[6]==1:
        ...
        xy_startend = [0,0,height,width]
        img_out_6 = cdalg.chronochrome_cd(img_multi_0, img_multi_1,alg='CC')
        # ^^^^^^Image_output_display^^^^^^#
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[7]==1:
        ...
        Tile_size = 1  # Since non-tile based algorithm
        xy_startend = [0,0,height,width]
        img_out_7 = cdalg.chronochrome_cd(img_multi_0, img_multi_1,alg='CE')
        # ^^^^^^Image_output_display^^^^^^#
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[8]==1:
        ...
        Tile_size = 1  # Since non-tile based algorithm
        xy_startend = [0,0,height,width]
        img_out = cdalg.cd_pca(img_multi_0, img_multi_1)
        # ^^^^^^Image_output_display^^^^^^#
        img_alg_out = Image.fromarray(img_out)
        img_alg_out = img_alg_out.resize((200, 200), Image.ANTIALIAS)
        img_alg_out = ImageTk.PhotoImage(img_alg_out)
    ...
    if ALG_CHECK[0]==1 or ALG_CHECK[9]==1:
        ...
        Tile_size = 1
        xy_startend = [0,0,height,width]
        img_out_9 = cdalg.image_diff(img_multi_0, img_multi_1)
        ##########################################################################################
        # OUTPUT_DISPLAY
        ...
        Tile_size = 1
        xy_startend = [0,0,height,width]
        img_out_10 = cdalg.image_ratio(img_multi_0, img_multi_1)
        ##########################################################################################
        # OUTPUT_DISPLAY
